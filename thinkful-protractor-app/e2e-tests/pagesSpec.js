...
describe("Test Pages", function() {
  ...
  it('should remove url entry on click of delete button', function() {
    createUrlEntry("url one", "http://url-one.com");
   ...
   it('should be able to edit the title of url entry', function() {
    createUrlEntry("url one", "http://url-one.com");

    browser.get(ROOT + "/");
    expect(element.all(by.css('.url-listing')).count()).toBe(1);

    element(by.xpath("//a[contains(text(),'Edit')]")).click().then(function() {
      browser.getLocationAbsUrl().then(function(url) {
        var title;
        expect(url).toMatch(/#\/edit\//);

        title = element(by.model('formCtrl.form.title'));
        expect(title.getAttribute('value')).toContain("url one");
        title.clear();
        title.sendKeys("url one updated");

        element(by.css('input[type=submit]')).click().then(function() {
          browser.getLocationAbsUrl().then(function(url) {
            expect(url).toMatch(/#\/urls/);
            expect(element.all(by.css('.url-listing')).count()).toBe(1);
            expect(element(by.css('.url-listing .listing-title')).getText()).toContain("url one updated");
          });
        });
      });
    });
